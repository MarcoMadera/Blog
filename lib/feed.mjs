...
async function generate() {
  const feed = new RSS({
    ...
    copyright: "2020 - 2022 Marco Madera",
    custom_namespaces: {
  ...
  const sortedDatapostData = getSortedPostData();
...
export function generateSearchData() {
  const sortedDatapostData = getSortedPostData();
  const searchData = sortedDatapostData.map(({ title, description, slug }) => ({
    title: title,
    description: description,
    url: `/blog/${slug}`,
  }));
