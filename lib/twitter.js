...
async function getTweetData(id, ignoreQuotedTweet = false) {
  ...
  if (res.body) {
    ...
    const quotedTweetId = Array.isArray(jsonred?.data?.referenced_tweets)
      ...
      : null;
    const repliedTweetId = Array.isArray(jsonred?.data?.referenced_tweets)
      ? jsonred?.data?.referenced_tweets[0]?.type === "replied_to" &&
        jsonred?.data?.referenced_tweets[0]?.id
      : null;
    ...
    if (repliedTweetId && !ignoreQuotedTweet) {
      const res = await getTweetData(
        repliedTweetId,
        (ignoreQuotedTweet = true)
      );
      repliedTweet = res;
    }
    return {
