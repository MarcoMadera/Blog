...
function Modal({
  ...
  return (
    <div className="modal-container">
      ...
      >
        <header>
          <H3 id="globalModalTitle">{title || "Modal"}</H3>
          <button
            type="button"
            tabIndex={0}
            onClick={(e) => {
      ...
      <style jsx>{`
        .modal {
          ...
          background-color: ${darkMode ? "#1f2937" : "#253a65"};
        }
      `}</style>
      <style jsx>{`
        .modal-container {
        ...
        .modal {
          border-radius: 8px;
          box-shadow: 0 4px 4px rgba(126, 126, 126, 0.3);
          color: #fff;
          z-index: 100;
...
export default function ModalContainer({
  ...
  useEffect(() => {
    setTargetNode(
      document.querySelector(
        props.modalRootId ? `#${props.modalRootId}` : "#global"
      ) as Element
    );
  ...
  useLayoutEffect(() => {
    const elementToBlur = document.querySelector<HTMLElement>("#__next");
